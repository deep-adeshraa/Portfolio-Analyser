...
public enum StockQuoteServiceFactory {
  ...
  public StockQuotesService getService(String provider,  RestTemplate restTemplate) {
    TiingoService tingo = new TiingoService(restTemplate);
    AlphavantageService alpha = new AlphavantageService(restTemplate);
    try {
      return provider.equalsIgnoreCase("tiingo") ?  tingo :  alpha;
    } catch (Exception e) {
      return alpha;
    }
  }
